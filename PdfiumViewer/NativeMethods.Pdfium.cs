...
namespace PdfiumViewer
{
  internal partial class NativeMethods
  {
    ...
    public static void FPDF_RenderPage(IntPtr dc, IntPtr page, int start_x, int start_y, int size_x, int size_y,
      int rotate, FPDF flags)
    {
      lock (LockString)
      {
    ...
    public static void FPDF_RenderPageBitmap(IntPtr bitmapHandle, IntPtr page, int start_x, int start_y, int size_x,
      int size_y, int rotate, FPDF flags)
    {
      lock (LockString)
      {
    ...
    public static void FPDFText_GetCharBox(IntPtr page, int index, out double left, out double right, out double bottom,
      out double top)
    {
      lock (LockString)
      {
    ...
    public static int FPDFText_GetCharIndexAtPos(IntPtr page, double x, double y, double xTolerance, double yTolerance)
    {
      lock (LockString)
      {
        int idx = Imports.FPDFText_GetCharIndexAtPos(page, x, y, xTolerance, yTolerance);
        if (idx == -3)
    ...
    public static List<PdfRectangle> FPDFText_GetRectangles(IntPtr page, int pageIndex, int startIndex,
      int characterCount)
    {
      lock (LockString)
      {
        ...
        int count = Imports.FPDFText_CountRects(page, startIndex, characterCount);
        List<PdfRectangle> rectangles = new List<PdfRectangle>(count);

        for (int i = 0; i < count; i++)
        {
          if (!Imports.FPDFText_GetRect(page, i, out double left, out double top, out double right, out double bottom))
            throw new PdfException((PdfError) Imports.FPDF_GetLastError());
    ...
    public static void FPDF_DeviceToPage(IntPtr page, int start_x, int start_y, int size_x, int size_y, int rotate,
      int device_x, int device_y, out double page_x, out double page_y)
    {
      lock (LockString)
      {
        Imports.FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, rotate, device_x, device_y, out page_x,
          out page_y);
      }
    }

    public static void FPDF_PageToDevice(IntPtr page, int start_x, int start_y, int size_x, int size_y, int rotate,
      double page_x, double page_y, out int device_x, out int device_y)
    {
      lock (LockString)
      {
        Imports.FPDF_PageToDevice(page, start_x, start_y, size_x, size_y, rotate, page_x, page_y, out device_x,
          out device_y);
      }
    }
    ...
    /// <param name="id">
    ///   Retrieves an IntPtr to the COM object for the Stream. The caller must release this with
    ///   Marshal.Release prior to Disposing the Stream.
    /// </param>
    ...
    private static class Imports
    {
      ...
      public static extern IntPtr FPDF_LoadCustomDocument([MarshalAs(UnmanagedType.LPStruct)] FPDF_FILEACCESS access,
        string password);
      ...
      [DllImport("pdfium.dll")]
      public static extern void FPDF_RenderPage(IntPtr dc, IntPtr page, int start_x, int start_y, int size_x,
        int size_y, int rotate, FPDF flags);

      [DllImport("pdfium.dll")]
      public static extern void FPDF_RenderPageBitmap(IntPtr bitmapHandle, IntPtr page, int start_x, int start_y,
        int size_x, int size_y, int rotate, FPDF flags);

      [DllImport("pdfium.dll")]
      public static extern int FPDF_GetPageSizeByIndex(IntPtr document, int page_index, out double width,
        out double height);
      ...
      [DllImport("pdfium.dll")]
      public static extern IntPtr FPDFText_FindStart(IntPtr page, byte[] findWhat, FPDF_SEARCH_FLAGS flags,
        int start_index);
      ...
      [DllImport("pdfium.dll")]
      public static extern void FPDFText_GetCharBox(IntPtr page, int index, out double left, out double right,
        out double bottom, out double top);

      [DllImport("pdfium.dll")]
      public static extern int FPDFText_GetCharIndexAtPos(IntPtr page, double x, double y, double xTolerance,
        double yTolerance);
      ...
      [DllImport("pdfium.dll")]
      public static extern bool FPDFText_GetRect(IntPtr page, int index, out double left, out double top,
      ...
      [DllImport("pdfium.dll")]
      public static extern void FPDF_DeviceToPage(IntPtr page, int start_x, int start_y, int size_x, int size_y,
      ...
      public static extern uint
        FPDFAction_GetURIPath(IntPtr document, IntPtr action, StringBuilder buffer, uint buflen);
      ...
      [DllImport("pdfium.dll")]
      public static extern bool FPDF_ImportPages(IntPtr destDoc, IntPtr srcDoc,
      ...
      [DllImport("pdfium.dll")]
      public static extern void FPDFPageObj_Transform(IntPtr page, double a, double b, double c, double d, double e,
        double f);
      ...
      [DllImport("pdfium.dll")]
      public static extern void FPDF_FFLDraw(IntPtr form, IntPtr bitmap, IntPtr page, int start_x, int start_y,
        int size_x, int size_y, int rotate, FPDF flags);
    ...
    public static void FPDF_FFLDraw(IntPtr form, IntPtr bitmap, IntPtr page, int start_x, int start_y, int size_x,
      int size_y, int rotate, FPDF flags)
    {
      lock (LockString)
      {
